	...
	public void send(final ReportRunStatus status) {
		try {
			...
			if (!configuration.mailPropertiesFile.exists()) {
				log.info("Email not sent because smtp properties file " + configuration.mailPropertiesFile
						+ " does not exist");
				status.finishEmail(null);
				return;
			}
			...
			if (email == null) {
				log.info("Email not sent because email request is missing");
				status.finishEmail(null);
				return;
			}
			final boolean success = status.getErrors().isEmpty();
			final boolean sendOnSuccess = email.getMailSuccess() != null ? email.getMailSuccess().booleanValue()
			...
			final boolean sendOnFailure = email.getMailFailure() != null ? email.getMailFailure().booleanValue()
			...
			log.info("emailPropertiesFile = " + configuration.mailPropertiesFile);
			...
			for (final String recipient : recipients) {
				...
				try {
					...
					final String subject = success
							? mailSuccessSubject.trim().isEmpty() ? "Success" : status.replace(mailSuccessSubject)
							: mailFailureSubject.trim().isEmpty() ? "Failure" : status.replace(mailFailureSubject);
					mimeMessage.setSubject(subject);
					final String mailSuccessBody = supercede(configuration.mailSuccessBody, email.getMailSuccessBody());
					final String mailFailureBody = supercede(configuration.mailFailureBody, email.getMailFailureBody());
					...
					final File outputFile = new File(configuration.outputDirectory, status.reportRun.outputFile);
